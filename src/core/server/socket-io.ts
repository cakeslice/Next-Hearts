...
import { DefaultEventsMap, EventNames } from 'socket.io/dist/typed-events.js'
...
export function socketMessage<ClientEvents extends DefaultEventsMap>(
	...
	const socket = global.io?.sockets.sockets.get(id) as Socket<DefaultEventsMap, ClientEvents>
	// @ts-ignore
...
export function socketBroadcast<ClientEvents extends DefaultEventsMap>(
	...
	// @ts-ignore
	roomId ? io?.to(roomId)?.emit(channel, message) : io?.emit(channel, message)
}
...
export function joinSocketRoom(
	...
	const socket = global.io?.sockets.sockets.get(id) as Socket
...
export function setupSocketEvents<ServerEvents extends DefaultEventsMap>(
	...
	const socket = global.io?.sockets.sockets.get(id) as Socket<ServerEvents, DefaultEventsMap>
